...
CAMLprim int aa_async_add_subscription(value o_client,
				       value o_uri,
				       value o_stream_id,
				       ...
				       value o_on_unavailable_image_opt)
{
  ...
  int err = aeron_async_add_subscription( &async, client, uri, stream_id,
					  aa_on_image, on_available_image,
					  aa_on_image, on_unavailable_image );
  if ( err == 0 ) {
